...
function one_click() {
    try{if(!!document.getElementById("menu_check").checked){
        document.getElementById("menu").style="background:#f8f8f8; box-shadow:0 0 6px 0 rgba(0,0,0,0.23);";
        document.getElementsByClassName("info")[0].style="display:block; position:fixed; top:72px; background:rgba(20,20,20,0.5);";
...
function two_click (){
try{    if(!!document.getElementById("menu_check2").checked && ( window.innerWidth <=600 )){
...
try{    document.getElementsByClassName("movement")[0].style="pointer-events:none;";  
    set(1,-1);}catch(e){}
}, 5000);
...
document.getElementsByClassName("pre")[0].onclick = function(){
    ...
    set(0,-1);}catch(err){return;}   
}
}catch(e){}
try{document.getElementsByClassName("next")[0].onclick = function(){
    try{document.getElementsByClassName("movement")[0].style="pointer-events:none;";
    if(!!interval){
        clearInterval(interval);
    }
